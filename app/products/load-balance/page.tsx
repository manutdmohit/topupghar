...
export default function LoadBalancePage() {
  const [loadBalanceProducts, setLoadBalanceProducts] = useState<Product[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [selectedCategory, setSelectedCategory] = useState<string>('all');

  useEffect(() => {
    const fetchLoadBalanceProducts = async () => {
      try {
        setLoading(true);
        const response = await fetch('/api/products/load-balance');

        if (!response.ok) {
          throw new Error('Failed to fetch load balance products');
        }

        const data = await response.json();
        setLoadBalanceProducts(data);
      } catch (err) {
        setError(err instanceof Error ? err.message : 'An error occurred');
        console.error('Error fetching load balance products:', err);
      } finally {
        setLoading(false);
      }
    };

    fetchLoadBalanceProducts();
  }, []);
    ...
    return [
      {
        ...
        icon: Scale,
        count: loadBalanceProducts.length,
      },
    ];
  };
            ...
            >
              <div className="bg-red-500/10 border border-red-500/20 rounded-2xl p-8 max-w-md mx-auto">
                ...
                <p className="text-red-400 text-lg mb-4">
                  Error loading load balance products
                </p>
                ...
                >
                  <div className="bg-white/5 backdrop-blur-sm rounded-2xl p-12 max-w-md mx-auto border border-white/10">
                    ...
                    <p className="text-gray-400">
                      No load balance products available at the moment.
                    </p>
                  </div>
                </motion.div>
              ) : (
        ...
        >
          <div className="bg-gradient-to-r from-purple-500/10 to-pink-500/10 backdrop-blur-sm rounded-3xl p-12 border border-white/10">
            <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">
              Ready to Load Balance?
            </h2>
            <p className="text-gray-300 text-lg mb-8 max-w-2xl mx-auto">
              Join thousands of users who trust us for their payment needs. Get
