...
export default function SocialMediaPage() {
  const [socialMediaProducts, setSocialMediaProducts] = useState<Product[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [selectedCategory, setSelectedCategory] = useState<string>('all');

  useEffect(() => {
    const fetchSocialMediaProducts = async () => {
      try {
        setLoading(true);
        const response = await fetch('/api/products/social-media');

        if (!response.ok) {
          throw new Error('Failed to fetch social media products');
        }

        const data = await response.json();
        setSocialMediaProducts(data);
      } catch (err) {
        setError(err instanceof Error ? err.message : 'An error occurred');
        console.error('Error fetching social media products:', err);
      } finally {
        setLoading(false);
      }
    };
    ...
    return [
      {
        ...
        icon: Share2,
        count: socialMediaProducts.length,
      },
    ];
  };
            ...
            >
              <div className="bg-red-500/10 border border-red-500/20 rounded-2xl p-8 max-w-md mx-auto">
                ...
                <p className="text-red-400 text-lg mb-4">
                  Error loading social media products
                </p>
                ...
                >
                  <div className="bg-white/5 backdrop-blur-sm rounded-2xl p-12 max-w-md mx-auto border border-white/10">
                    ...
                    <p className="text-gray-400">
                      No social media products available in this category at the
                      moment.
                    </p>
                  </div>
                </motion.div>
        ...
        >
          <div className="bg-gradient-to-r from-purple-500/10 to-pink-500/10 backdrop-blur-sm rounded-3xl p-12 border border-white/10">
            ...
            <p className="text-gray-300 text-lg mb-8 max-w-2xl mx-auto">
              Join thousands of users who trust us for their social media growth. Get
              instant delivery and boost your online presence!
            </p>
