...
export default function LinkedInPremiumPage() {
  ...
  const [product, setProduct] = useState<LinkedInProduct | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const router = useRouter();

  useEffect(() => {
    const fetchLinkedInProduct = async () => {
      try {
        setLoading(true);
        const response = await fetch('/api/products/linkedin');

        if (!response.ok) {
          throw new Error('Failed to fetch LinkedIn product');
        }

        const data = await response.json();
        setProduct(data);
      } catch (err) {
        console.error('Error fetching LinkedIn product:', err);
    ...
    return (
      <div className="max-w-xl mx-auto px-4 py-10">
        <div className="text-center py-8">
          ...
          <p className="text-gray-600">Loading LinkedIn packages...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="max-w-xl mx-auto px-4 py-10">
      ...
      <div className="text-center mb-10">
        ...
        <div className="mt-6 flex justify-center">
          <Image
            src={product.image || '/linkedin.jpg'}
        ...
        {product.variants.map((variant, index) => (
          ...
          >
            <h3 className="text-xl font-semibold text-purple-700">
              LinkedIn Premium ({variant.label})
            </h3>
            <p className="text-gray-700 mt-2 font-medium">
              NPR {variant.price.toLocaleString('en-US')}
            </p>
            <div className="mt-2 text-xs text-gray-500">
              Full access | {variant.duration}
            </div>
            {selectedPackage === index + 1 && (
