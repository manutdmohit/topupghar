...
interface YouComVariant {
  label: string;
  duration: string;
  price: number;
}

interface YouComProduct {
  _id: string;
  name: string;
  platform: string;
  type: string;
  description: string;
  image: string;
  variants: YouComVariant[];
  isActive: boolean;
}

export default function YouComPage() {
  const [selectedPackage, setSelectedPackage] = useState<number | null>(null);
  const [product, setProduct] = useState<YouComProduct | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const router = useRouter();

  useEffect(() => {
    const fetchYouComProduct = async () => {
      try {
        setLoading(true);
        const response = await fetch('/api/products/you.com');

        if (!response.ok) {
          throw new Error('Failed to fetch You.com product');
        }

        const data = await response.json();
        setProduct(data);
      } catch (err) {
        console.error('Error fetching You.com product:', err);
    ...
    return (
      <div className="max-w-xl mx-auto px-4 py-10">
        <div className="text-center py-8">
          ...
          <p className="text-gray-600">Loading You.com packages...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="max-w-xl mx-auto px-4 py-10">
      ...
      <div className="text-center mb-10">
        <h1 className="text-4xl font-bold text-purple-700">
          {product.name} – 1 Year Subscription
        </h1>
        <p className="text-gray-600 mt-2">{product.description}</p>
        <div className="mt-6 flex justify-center">
          <Image
            src={product.image || '/you-logo.jpg'}
            alt="You.com Premium"
            width={500}
            height={500}
        ...
        {product.variants.map((variant, index) => (
          ...
          >
            ...
            <div className="mt-2 text-xs text-gray-500">
              {variant.duration} Premium Subscription
            </div>
            {selectedPackage === index + 1 && (
