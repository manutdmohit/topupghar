...
interface CourseraVariant {
  label: string;
  duration: string;
  price: number;
}

interface CourseraProduct {
  _id: string;
  name: string;
  platform: string;
  type: string;
  description: string;
  image: string;
  variants: CourseraVariant[];
  isActive: boolean;
}

export default function CourseraPlusPage() {
  const [selectedPackage, setSelectedPackage] = useState<number | null>(null);
  const [product, setProduct] = useState<CourseraProduct | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const router = useRouter();

  useEffect(() => {
    const fetchCourseraProduct = async () => {
      try {
        setLoading(true);
        const response = await fetch('/api/products/coursera-plus');

        if (!response.ok) {
          throw new Error('Failed to fetch Coursera Plus product');
        }

        const data = await response.json();
        setProduct(data);
      } catch (err) {
        console.error('Error fetching Coursera Plus product:', err);
    ...
    return (
      <div className="max-w-xl mx-auto px-4 py-10">
        <div className="text-center py-8">
          ...
          <p className="text-gray-600">Loading Coursera Plus packages...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="max-w-xl mx-auto px-4 py-10">
      ...
      <div className="text-center mb-10">
        ...
        <div className="mt-6 flex justify-center">
          <Image
            src={product.image || '/coursera-plus.jpg'}
            alt="Coursera Plus"
            width={500}
            height={500}
        ...
        {product.variants.map((variant, index) => (
          ...
          >
            ...
            <div className="mt-2 text-xs text-gray-500">
              {variant.duration} Access | Unlimited Courses & Certificates
            </div>
            {selectedPackage === index + 1 && (
