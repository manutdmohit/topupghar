...
export default function FigmaProfessionalPage() {
  const [selectedPackage, setSelectedPackage] = useState<number | null>(null);
  const [product, setProduct] = useState<FigmaProduct | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const router = useRouter();

  useEffect(() => {
    const fetchFigmaProduct = async () => {
      try {
        setLoading(true);
        const response = await fetch('/api/products/figma');

        if (!response.ok) {
          throw new Error('Failed to fetch Figma product');
        }

        const data = await response.json();
        setProduct(data);
      } catch (err) {
        console.error('Error fetching Figma product:', err);
    ...
    return (
      <div className="max-w-xl mx-auto px-4 py-10">
        <div className="text-center py-8">
          ...
          <p className="text-gray-600">Loading Figma packages...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="max-w-xl mx-auto px-4 py-10">
      ...
      <div className="text-center mb-10">
        <h1 className="text-4xl font-bold text-purple-700">
          Figma Professional – 1 Year
        </h1>
        <p className="text-gray-600 mt-2">
          Unlock Figma’s professional features for a full year. Fast delivery
          after payment.
        </p>
        ...
        {product.variants.map((variant, index) => (
          ...
          >
            ...
            <div className="mt-2 text-xs text-gray-500">
              Professional Plan | {variant.duration}
            </div>
            {selectedPackage === index + 1 && (
