...
export default function ChatGptYearlyPage() {
  ...
  useEffect(() => {
    const fetchChatGptProduct = async () => {
      try {
        setLoading(true);
        const response = await fetch('/api/products/chatgpt-plus-one-year');

        if (!response.ok) {
          throw new Error('Failed to fetch ChatGPT Plus One Year product');
        }

        const data = await response.json();
        setProduct(data);
      } catch (err) {
        console.error('Error fetching ChatGPT Plus One Year product:', err);
      ...
      <div className="max-w-xl mx-auto px-4 py-10">
        <div className="text-center py-8">
          ...
          <p className="text-gray-600">
            Loading ChatGPT Plus One Year packages...
          </p>
        </div>
      </div>
    );
    ...
    <div className="max-w-xl mx-auto px-4 py-10">
      ...
      <div className="text-center mb-10">
        ...
        <div className="mt-6 flex justify-center">
          <Image
            src={product.image || '/chatgpt-plus-one-year-banner.jpg'}
            alt="ChatGPT"
            width={250}
            height={250}
